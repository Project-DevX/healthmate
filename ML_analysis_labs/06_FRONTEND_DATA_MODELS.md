# Frontend Data Models - Trend Analysis Data Classes

## Overview

This document contains all the data model classes for handling trend analysis data in Flutter. These models provide type safety, data validation, and convenient methods for working with trend data.

## Implementation

### Create Data Models File

**File:** `lib/models/trend_data_models.dart`
...
class TrendAnalysisData {
  ...
  TrendSummary get summary {
    return TrendSummary(
      ...
      significantTrends: vitals.values.where((v) => v.trendSignificance > 0.6).length,
      anomaliesDetected: vitals.values.fold(0, (sum, v) => sum + v.anomalies.length),
      predictionsAvailable: predictions.values.fold(0, (sum, p) => sum + p.length),
...
final trendData = TrendAnalysisData.fromFirestore(firestoreDocument.data());

// Access vital trends
for (final vitalEntry in trendData.vitals.entries) {
  final vitalName = vitalEntry.key;
  final vitalTrend = vitalEntry.value;
  
  print('${vitalName}: ${vitalTrend.formattedTrendDirection}');
  print('Current: ${vitalTrend.formattedCurrentValue}');
  print('Trend: ${vitalTrend.trendInterpretation}');
}
```
...
final bloodSugarPredictions = trendData.predictions['glucose'] ?? [];
for (final prediction in bloodSugarPredictions) {
  print('${prediction.formattedDate}: ${prediction.formattedPredictedValue('mg/dL')}');
...
print('Status: ${summary.healthStatus.displayName} ${summary.healthStatus.emoji}');
print('Significant Trends: ${summary.significantTrends}');
print('Anomalies: ${summary.anomaliesDetected}');
```

### Working with Anomalies

```dart
// Check for anomalies in vitals
for (final vital in trendData.vitals.values) {
  if (vital.anomalies.isNotEmpty) {
    print('⚠️ ${vital.vitalName} has ${vital.anomalies.length} anomalies');
    
    for (final anomaly in vital.anomalies) {
      print('- Value: ${anomaly.value}, Z-Score: ${anomaly.formattedZScore}');
      print('- Severity: ${anomaly.severityLevel.name}');
    }
  }
}
```
...
Continue to **07_FRONTEND_UI_COMPONENTS.md** to implement the user interface components that use these data models.
